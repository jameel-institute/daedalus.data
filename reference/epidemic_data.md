...
`epidemic_names` provides the tags by which epidemics are known for
users' convenience.
...
A list with 7 elements, each corresponding to an epidemic (see
**Details**), and providing 10 infection parameters:
...
- `p_sigma`: A single numeric value in the range \\(0.0, 1.0)\\ for the
  proportion of infectious individuals who are also symptomatic.
...
- `gamma_H_death`: A single numeric value for the rate of transition
  from hospital admission to death.

- `hfr`: A numeric vector of length 4 for the age-specific hospital
  fatality ratio.
